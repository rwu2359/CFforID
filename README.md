...
To convert back from FMFID to the Field Id, just do the reverse. Take off the table ID, divide by 2^32 and then look that value up in the field names -- alternatively use Div ( FMFID ; 2^32 ) as that does not need you to find the tableID first (thanks @mrwatson-de).  While you are helpgin yourself to learning, better go and read this excellent article by Andrew Duncan from databuzz about [querying the system tables](https://www.databuzz.com.au/accessing-base-tables-in-claris-filemaker-2023/)
...
## FileMaker file
Simple demo with some layout calculations to show the custom functions in operation.
